...
change_station_prev(GtkWidget * widget, GdkEvent * event,
    ...
    valid =
        gtk_tree_model_get_iter(GTK_TREE_MODEL(app->user_stations_list),
                                &iter, path);
    while (valid) {
        ...
        if (ready) {
            ...
            if (!valid) {
                ...
                while (gtk_tree_model_iter_next
                       (GTK_TREE_MODEL(app->user_stations_list), &iter));
                path =
...
int calculate_offset_of_day(int count_day) {
    ...
    long int diff_time;
    int year, current_month, offset;
    time_t current_day, current_time, date_time;
    struct tm *tm = NULL;
    double pre_offset = 0.0F;
    gchar date_in_string[255];
    GSList *tmp = NULL, *first_day = NULL, *last_day = NULL;
...
void draw_home_window(gint count_day) {
    gint i, offset = 0, year, current_month = 1;
    gchar buffer[2048],
        buffer_icon[1024], temperature_string[1024], forecast_string[2048];
    time_t current_day,
        current_time, update_time, last_day =
        0, day_begin_time, night_begin_time;
    long int diff_time;
    struct tm *tm = NULL, tmp_tm;
    gboolean flag_last_day = FALSE, is_na_day = FALSE;
    gint icon_size;
    gchar *tmp_station_name;
    WDB *tmp_button = NULL;
    GSList *tmp = NULL,
        *day = NULL, *tmp_day = NULL, *first = NULL, *last = NULL;
#ifndef RELEASE
    time_t tmp_time, utc_time;
#endif
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
...
/* get current day */
    ...
    while (i < app->config->days_to_show) {
        if (tmp) {
            ...
            /* day begin */
            day_begin_time =
                get_day_part_begin_time(day, year, "day_sunrise");
            /* night begin */
            night_begin_time =
                get_day_part_begin_time(day, year, "day_sunset");
            ...
            if (i == 0) {       /* first day */
                ...
                if (!app->wsd.current_data_is_invalid &&
                    ...
                    event_add(update_time +
                              app->config->data_valid_interval -
                              diff_time, CHANGE_DAY_PART);
                    buffer[0] = 0;
                    create_current_temperature_text(app->wsd.current,
                                                    buffer, TRUE,
                                                    item_value(day,
                                                               "day_name"));
                    ...
                    if (app->config->separate) {        /* if current data isn't actual and first day */
                        ...
                        create_day_temperature_text(day, buffer, FALSE,
                                                    FALSE);
            ...
            if (app->config->show_wind)
                sprintf(buffer,
                        ...
                        app->config->font_color.blue >> 8, _("N/A"),
                        _("N/A"), _("N/A"), _("N/A"), _("N/A"));
            else
                sprintf(buffer,
                        "<span foreground='#%02x%02x%02x'>%s\n%s\302\260\n%s\302\260</span>",
                        app->config->font_color.red >> 8,
        ...
        if (app->config->icons_layout == COMBINATION && i == 0) /* First icon in COMBINATION layout */
            tmp_button =
                create_weather_day_button(buffer, buffer_icon,
                                          icon_size * 2,
            ...
            tmp_button =
                create_weather_day_button(buffer, buffer_icon, icon_size,
                                          app->config->transparency, TRUE,
                                          &(app->
                                            config->background_color));
        g_signal_connect(tmp_button->button, "button-release-event",
                         G_CALLBACK(weather_window_popup),
                         (is_na_day ? (GINT_TO_POINTER(-1))
                          : (GINT_TO_POINTER(i))));
    ...
    if (app->config->use_sensor && app->config->display_at == ICON)     /* draw sensor data at the new icon */
...
/* creating main panel */
    ...
    gtk_box_pack_start(GTK_BOX(app->top_widget), app->main_window, TRUE,
                       TRUE, 0);
    gtk_widget_show_all(app->top_widget);
#ifdef OS2008
    if (!app->config->transparency && app->parent)
        gtk_widget_modify_bg(app->parent, GTK_STATE_NORMAL,
...
/* Init gconf. */
    ...
    parse_template_from_file
        ("/usr/share/omweather/templates/weather_com/popup.xml", "UTF-8");
...
/* Start main applet */
    ...
    applet->priv =
        G_TYPE_INSTANCE_GET_PRIVATE(applet, TYPE_OMWEATHER,
                                    OMWeatherPrivate);
...
void menu_init(void) {
    ...
    while (valid) {
        gtk_tree_model_get(GTK_TREE_MODEL(app->user_stations_list),
                           &iter, 0, &station_name, 1, &station_code, -1);
        if (station_name) {
    ...
    gtk_widget_tap_and_hold_setup(app->main_window,
                                  GTK_WIDGET(app->contextmenu), NULL, 0);
}
...
GtkWidget *create_forecast_weather_simple_widget(GSList * day) {
    GtkWidget *temperature_label = NULL,
        *main_data_vbox = NULL,
        *main_data_label = NULL, *main_widget = NULL, *temperature_vbox =
        NULL;
    gchar buffer[1024];
    time_t day_begin_time, night_begin_time, current_time;
    gint year;
    struct tm *tm;
    ...
    gtk_label_set_justify(GTK_LABEL(temperature_label),
                          GTK_JUSTIFY_CENTER);

    set_font(temperature_label, app->config->font, -1);
    gtk_box_pack_start(GTK_BOX(temperature_vbox), temperature_label,
                       FALSE, FALSE, 0);
    ...
    sprintf(buffer, "<span weight=\"bold\" foreground='#%02x%02x%02x'>",
            ...
            app->config->font_color.blue >> 8);
    if ((current_time > day_begin_time)
        && (current_time < night_begin_time)) {
        strcat(buffer, (gchar *)
               hash_table_find(item_value(day, "day_title"), FALSE));
        strcat(buffer, _("\nHumidity: "));
        ...
        sprintf(buffer + strlen(buffer), "%s %.2f %s", (gchar *)
                hash_table_find(item_value(day, "day_wind_title"),
                                TRUE),
                convert_wind_units(app->config->wind_units,
                                   atof(item_value
                                        (day, "day_wind_speed"))),
                (gchar *) hash_table_find((gpointer)
                                          wind_units_str[app->config->
                                                         wind_units],
                                          FALSE));
    } else {
        strcat(buffer, (gchar *)
               hash_table_find(item_value(day, "night_title"), FALSE));
        strcat(buffer, _("\nHumidity: "));
        ...
        sprintf(buffer + strlen(buffer), "%s %.2f %s",
                item_value(day, "night_wind_title"),
                convert_wind_units(app->config->wind_units,
                                   atof(item_value
    ...
    gtk_box_pack_start(GTK_BOX(main_data_vbox), main_data_label, FALSE,
                       FALSE, 0);
    /* prepare main widget */
    main_widget = gtk_hbox_new(FALSE, 10);
    gtk_box_pack_start(GTK_BOX(main_widget), temperature_vbox, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(main_widget), main_data_vbox, FALSE,
                       FALSE, 0);
...
GtkWidget *create_current_weather_simple_widget(GSList * current) {
    GtkWidget *main_widget = NULL,
        *temperature_vbox,
        *temperature_label,
        *main_data_vbox, *main_data_label, *icon_temperature_vbox;
...
/* prepare current temperature */
    ...
    (app->config->temperature_units ==
     CELSIUS) ? (strcat(buffer, _("C"))) : (strcat(buffer, _("F")));
    strcat(buffer, "</span>");
    temperature_label = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(temperature_label), buffer);
    gtk_label_set_justify(GTK_LABEL(temperature_label),
...
/* feels like */
    ...
    sprintf(buffer + strlen(buffer), "%d\302\260",
            (app->config->temperature_units ==
             CELSIUS) ? (atoi(item_value(current, "feel_like")))
            : ((int)
               c2f(atof(item_value(current, "feel_like")))));
    (app->config->temperature_units ==
     CELSIUS) ? (strcat(buffer, _("C"))) : (strcat(buffer, _("F")));
/* humidity */
    strcat(buffer, _(" H: "));
...
/* visible */
    ...
    else if (strcmp(item_value(current, "visible"), "N/A")) {
...
/* prepare icon and temperature vbox */
    ...
    gtk_box_pack_start(GTK_BOX(icon_temperature_vbox), temperature_vbox,
                       FALSE, FALSE, 0);
/* prepare main widget */
    main_widget = gtk_hbox_new(FALSE, 10);
    gtk_box_pack_start(GTK_BOX(main_widget), icon_temperature_vbox,
                       FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(main_widget), main_data_vbox, FALSE,
                       FALSE, 0);
#ifdef DEBUGFUNCTIONCALL
...
create_panel(GtkWidget * panel, gint layout, gboolean transparency,
             gchar * st_name) {
    gchar buffer[2048];
    GtkWidget *header_panel = NULL, *days_panel = NULL,
/*		*days_panel_with_buttons = NULL,*/
        ...
        *station_name_btn = NULL, *station_name = NULL, *station_box =
        NULL;
    time_t current_time, update_time, diff_time;

    int n, elements, x, y, total_elements = 0;
    GtkTreeIter iter;
    gboolean valid = FALSE, user_stations_list_has_two_or_more_elements =
        FALSE;
    GSList *tmp = NULL;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
...
/* draw arrows */
    if (app->config->show_arrows
        && user_stations_list_has_two_or_more_elements) {
        ...
        set_background_color(previos_station_name_btn,
                             &(app->config->background_color));
        ...
        gtk_label_set_justify(GTK_LABEL(previos_station_name),
                              GTK_JUSTIFY_CENTER);
        set_font(previos_station_name, app->config->font, -1);
        gtk_box_pack_start((GtkBox *) previos_station_box,
                           previos_station_name, TRUE, TRUE, 15);
        gtk_container_add(GTK_CONTAINER(previos_station_name_btn),
                          previos_station_box);
        ...
        set_background_color(next_station_name_btn,
                             &(app->config->background_color));

        gtk_widget_set_events(next_station_name_btn,
                              GDK_BUTTON_RELEASE_MASK |
                              GDK_BUTTON_PRESS_MASK);
        next_station_name = gtk_label_new(NULL);
        gtk_label_set_markup(GTK_LABEL(next_station_name), buffer);
        gtk_label_set_justify(GTK_LABEL(next_station_name),
                              GTK_JUSTIFY_CENTER);
        set_font(next_station_name, app->config->font, -1);
        gtk_box_pack_start((GtkBox *) next_station_box,
                           next_station_name, TRUE, TRUE, 15);
        gtk_container_add(GTK_CONTAINER(next_station_name_btn),
                          next_station_box);
    }
    buffer[0] = 0;
...
/* create station name button */
...
#if defined(OS2008) || defined(DEBUGTEMP)
        {
            if (app->config->use_sensor && app->config->display_at == STATION_NAME)     /* draw sensor data at the station name */
                sprintf(buffer,
        ...
        set_background_color(station_name_btn,
                             &(app->config->background_color));
        gtk_widget_set_events(station_name_btn,
                              GDK_BUTTON_RELEASE_MASK |
                              GDK_BUTTON_PRESS_MASK);
        ...
        gtk_box_pack_start((GtkBox *) station_box, station_name, TRUE,
                           TRUE, 0);
    ...
    if (next_station_name_btn)
        gtk_event_box_set_visible_window(GTK_EVENT_BOX
                                         (next_station_name_btn), FALSE);
    if (station_name_btn)
        gtk_event_box_set_visible_window(GTK_EVENT_BOX
                                         (station_name_btn), FALSE);
#else
...
/* attach days buttons */
    ...
    if (layout == COMBINATION) {
        ...
        if (!app->wsd.current_data_is_invalid &&
            ...
            if (app->wsd.days)
                current_weather_widget
                    = create_forecast_weather_simple_widget(((GSList *)
                                                             (app->
                                                              wsd.days))->
                                                            data);
        }
        gtk_box_pack_start(GTK_BOX(combination_vbox), header_panel,
                           FALSE, FALSE, 0);
        if (current_weather_widget)
            gtk_box_pack_start(GTK_BOX(combination_vbox),
                               current_weather_widget, FALSE, FALSE, 0);
        gtk_table_attach((GtkTable *) days_panel, combination_vbox, 1,
                         Max_count_weather_day, 0, 1,
...
WDB *create_weather_day_button(const char *text, const char *icon,
    ...
    gtk_widget_set_events(new_day_button->button,
                          GDK_BUTTON_RELEASE_MASK | GDK_BUTTON_PRESS_MASK);
#ifdef OS2008
    gtk_event_box_set_visible_window(GTK_EVENT_BOX
                                     (new_day_button->button), FALSE);
#else
    ...
    if (draw_day_label) {
        ...
        gtk_label_set_justify(GTK_LABEL(new_day_button->label),
                              GTK_JUSTIFY_RIGHT);
    ...
    new_day_button->icon_buffer =
        gdk_pixbuf_new_from_file_at_size(icon, icon_size, icon_size, NULL);
    ...
    gtk_container_add(GTK_CONTAINER(new_day_button->button),
                      new_day_button->box);
...
get_day_part_begin_time(GSList * day, guint year, const gchar * day_part) {
    ...
    memset(buffer, 0, sizeof(buffer));
    snprintf(buffer, sizeof(buffer) - 1,
             "%s %i %s", item_value(day, "day_date"), year,
             item_value(day, day_part));
...
void add_wind_text(GSList * day, gchar * buffer) {
    ...
    if (strcmp((char *)item_value(day, "day_wind_speed"), "N/A")) {
        if (!strcmp((char *)item_value(day, "day_wind_title"), "N/A"))
            ...
            wind_direction = (char *)
                hash_table_find(item_value(day, "day_wind_title"), TRUE);
        sprintf(buffer + strlen(buffer),
                "<span foreground='#%02x%02x%02x'>\n%s\n%.1f</span>",
                app->config->font_color.red >> 8,
                app->config->font_color.green >> 8,
                app->config->font_color.blue >> 8, wind_direction,
                convert_wind_units(app->config->wind_units,
                                   atof(item_value
                                        (day, "day_wind_speed"))));
    } else
        sprintf(buffer + strlen(buffer),
                ...
                app->config->font_color.red >> 8,
                app->config->font_color.green >> 8,
                app->config->font_color.blue >> 8, _("N/A"), _("N/A"));
}

/*******************************************************************************/
void
create_current_temperature_text(GSList * day, gchar * buffer,
                                gboolean valid, const gchar * day_name) {
...
create_day_temperature_text(GSList * day, gchar * buffer, gboolean valid,
    ...
    gint temp_hi = INT_MAX, temp_low = INT_MAX;
...
/*******************************************************************************/
#ifdef OS2008
GtkWidget *settings_menu(HildonDesktopHomeItem * home_item,
                         GtkWindow * parent) {
    GtkWidget *menu_item = NULL;
