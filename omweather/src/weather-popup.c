...
GtkWidget *create_sun_time_widget(GSList * day) {
    GtkWidget *main_widget = NULL, *main_label;
    gchar buffer[1024], temp_buffer[1024], time_buffer[1024];
...
/* prepare additional time values */
    ...
    snprintf(buffer + strlen(buffer),
             sizeof(buffer) - strlen(buffer) - 1, " %s", _("Sunset: "));
...
GtkWidget *create_moon_phase_widget(GSList * current) {
    GtkWidget *main_widget = NULL, *main_label = NULL;
    gchar buffer[1024], icon[50], *space_symbol = NULL;
    ...
    memset(buffer, 0, sizeof(buffer));
    snprintf(buffer + strlen(buffer),
             sizeof(buffer) - strlen(buffer) - 1, "%s", (char *)
             hash_table_find(item_value(current, "moon_phase"), FALSE));
    main_label = gtk_label_new(buffer);
    set_font(main_label, NULL, 14);

    main_widget = gtk_hbox_new(FALSE, 0);
/* Moon icon */
    sprintf(icon, "%s%s.png", MOON_ICONS,
            item_value(current, "moon_phase"));
    ...
    if (icon_image)
        gtk_box_pack_start(GTK_BOX(main_widget), icon_image, FALSE,
                           TRUE, 0);
...
GtkWidget *create_time_updates_widget(GSList * current) {
    GtkWidget *main_widget = NULL, *label_update;
    ...
    if (stat(full_filename, &statv)) {
        ...
        strftime(buffer + strlen(buffer),
                 sizeof(buffer) - strlen(buffer) - 1, "%X %x",
                 localtime(&statv.st_mtime));
        snprintf(buffer + strlen(buffer),
                 sizeof(buffer) - strlen(buffer) - 1, " %s",
                 _("current location local time"));
    }
...
/*******************************************************************************/
gboolean
weather_window_popup(GtkWidget * widget, GdkEvent * event,
                     gpointer user_data) {
    GtkWidget *notebook = NULL,
        ...
        *about_button = NULL,
        *close_button = NULL,
        *label_box = NULL, *copyright_box = NULL, *no_weather_box = NULL;
    gint active_tab = 0, k = 0, page = 0, i = 1;
    gchar *day_name = NULL;
    time_t current_time = 0,
        diff_time, current_data_last_update = 0, data_last_update = 0;
    GSList *tmp = NULL, *day = NULL;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
...
/* Main window */
    ...
    g_object_set_data(G_OBJECT(app->popup_window), "active_tab",
                      (gpointer) active_tab);
...
/* station name */
    ...
    gtk_container_add(GTK_CONTAINER(label_box),
                      create_window_header(app->
                                           config->current_station_name,
                                           app->popup_window));
...
/* correct time for current location */
    diff_time =
        calculate_diff_time(atol
                            (item_value
                             (app->wsd.location, "station_time_zone")));
#ifndef RELEASE
    fprintf(stderr, "\n>>>>>>>Diff time=%li<<<<<<\n", diff_time);
#endif
...
/* Detailed weather tab */
    if (!app->wsd.hours_data_is_invalid && app->wsd.hours_weather) {
        data_last_update =
            last_update_time((GSList *) (app->wsd.hours_weather)->data);
        /* Check a valid time for hours forecast */
        if (app->config->show_weather_for_two_hours
            && (!app->wsd.current_data_is_invalid)
    ...
    while (tmp && i < app->config->days_to_show + k) {
        ...
        if (active_tab != i) {
            ...
            if (tab) {
                page =
                    gtk_notebook_append_page(GTK_NOTEBOOK(notebook),
                                             tab, gtk_label_new(day_name));
                g_object_set_data(G_OBJECT(tab), "day",
                                  (gpointer) tmp->data);
...
/* Bottom buttons box */
    ...
    settings_button =
        create_button_with_image(BUTTON_ICONS, "settings", 40, FALSE,
                                 FALSE);
    ...
    refresh_button =
        create_button_with_image(BUTTON_ICONS, "refresh", 40, FALSE,
                                 FALSE);
...
/* Pack buttons to the buttons box */
    ...
    gtk_box_pack_start(GTK_BOX(buttons_box), settings_button, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(buttons_box), refresh_button, FALSE,
                       FALSE, 0);
    gtk_box_pack_start(GTK_BOX(buttons_box), about_button, FALSE, FALSE,
                       0);
    gtk_box_pack_start(GTK_BOX(buttons_box), close_button, FALSE, FALSE,
                       0);
    ...
    if (gtk_notebook_get_n_pages(GTK_NOTEBOOK(notebook)) > 0) {
        ...
        gtk_container_add(GTK_CONTAINER(no_weather_box), label =
                          gtk_label_new(_
                                        ("No weather data for this station.")));
        gtk_box_pack_start(GTK_BOX(vbox), no_weather_box, TRUE, TRUE, 0);
        gtk_event_box_set_visible_window(GTK_EVENT_BOX(no_weather_box),
                                         FALSE);
...
/* Show copyright widget */
    ...
    gtk_container_add(GTK_CONTAINER(copyright_box),
                      create_copyright_widget(weather_sources
                                              [app->config->
                                               current_station_source].name,
                                              NULL));
    gtk_box_pack_start(GTK_BOX(vbox), copyright_box, FALSE, TRUE, 0);
    ...
    if (app->config->theme_override_in_use) {
        ...
        gtk_rc_reset_styles(gtk_settings_get_for_screen
                            (gdk_screen_get_default()));
        if (GTK_IS_NOTEBOOK(notebook)) {
            for (i = 0;
                 i < gtk_notebook_get_n_pages(GTK_NOTEBOOK(notebook));
                 i++) {
        ...
        g_signal_connect(G_OBJECT(no_weather_box), "expose-event",
                         G_CALLBACK(draw_top_gradient), NULL);

        gtk_widget_modify_bg(copyright_box, GTK_STATE_NORMAL, &color);
        g_signal_connect(G_OBJECT(label_box), "expose-event",
                         G_CALLBACK(draw_label_gradient), NULL);
...
/*******************************************************************************/
void
refresh_button_handler(GtkWidget * button, GdkEventButton * event,
                       gpointer user_data) {
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
...
/*******************************************************************************/
void
about_button_handler(GtkWidget * button, GdkEventButton * event,
                     gpointer user_data) {
    create_about_dialog();
}
...
/*******************************************************************************/
void
popup_close_button_handler(GtkWidget * button, GdkEventButton * event,
                           gpointer user_data) {
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
...
/*******************************************************************************/
GtkWidget *create_pseudo_day_tab(GSList * current, GSList * day,
                                 gchar ** day_name) {
...
/*******************************************************************************/
GtkWidget *create_day_tab(GSList * current, GSList * day,
                          gchar ** day_name) {
    ...
    GtkWidget *main_widget = NULL,
        ...
        *day_text_vbox = NULL,
        *night_text_vbox = NULL, *day_text = NULL, *night_text = NULL;
    gchar buffer[1024], symbol = 'C';
    struct tm tmp_time_date_struct = { 0 };
    GdkPixbuf *icon = NULL;
    gint hi_temp, low_temp, day_invalid_count = 0, night_invalid_count = 0;
    ...
    *day_name = g_strdup(buffer);
    /* prepare title */
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "%s %s", item_value(day, "day_name"),
            item_value(day, "day_date"));
    strptime(buffer, "%A %b %d", &tmp_time_date_struct);
    memset(buffer, 0, sizeof(buffer));
    strftime(buffer, sizeof(buffer) - 1, "%A, %d %B",
             &tmp_time_date_struct);
    ...
    gtk_box_pack_start(GTK_BOX(day_vbox),
                       day_label_temperature_hbox, TRUE, TRUE, 0);
    /* prepare day label */
    ...
    if (hi_temp == INT_MAX)
        strncat(buffer, (char *)hash_table_find("N/A", FALSE),
                ((strlen((char *)hash_table_find("N/A", FALSE)) >
                  sizeof(buffer)) ? (sizeof(buffer) - 1) : (strlen((char *)
                                                                   hash_table_find("N/A", FALSE)))));
    else
        snprintf(buffer, sizeof(hi_temp) +
                 ((strlen(("\302\260%c")) > sizeof(buffer)) ?
                  (sizeof(buffer) -
                   1) : (strlen("\302\260%c"))), "%d\302\260%c",
                 hi_temp, symbol);
    gtk_box_pack_start(GTK_BOX(day_label_temperature_hbox),
                       gtk_label_new(buffer), FALSE, TRUE, 40);
    /* hbox for icon and text */
    day_icon_text_hbox = gtk_hbox_new(FALSE, 0);
    gtk_box_pack_start(GTK_BOX(day_vbox), day_icon_text_hbox, TRUE,
                       TRUE, 0);
    /* day icon */
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "%s%s.png", path_large_icon,
            item_value(day, "day_icon"));
    icon =
        gdk_pixbuf_new_from_file_at_size(buffer, BIG_ICON_SIZE,
                                         BIG_ICON_SIZE, NULL);
    day_icon = gtk_image_new_from_pixbuf(icon);
    if (icon)
        g_object_unref(icon);
    gtk_box_pack_start(GTK_BOX(day_icon_text_hbox), day_icon, TRUE,
                       TRUE, 5);
/* preapare day text */
    gtk_box_pack_start(GTK_BOX(day_icon_text_hbox),
                       day_text_vbox =
                       gtk_vbox_new(FALSE, 0), TRUE, TRUE, 0);
    memset(buffer, 0, sizeof(buffer));
    if (!strcmp((char *)item_value(day, "day_title"), "N/A"))
        day_invalid_count++;
    strcat(buffer,
           (char *)hash_table_find(item_value(day, "day_title"), FALSE));
    strcat(buffer, _("\nHumidity: "));
    ...
    if (!strcmp((char *)item_value(day, "day_wind_speed"), "N/A"))
        day_invalid_count++;
    sprintf(buffer + strlen(buffer), "%s\n%.2f %s", (char *)
            hash_table_find(item_value(day, "day_wind_title"), FALSE),
            convert_wind_units(app->config->wind_units,
                               atof(item_value
                                    (day, "day_wind_speed"))),
            (char *)hash_table_find((gpointer)
                                    wind_units_str[app->
                                                   config->wind_units],
                                    FALSE));
    ...
    gtk_box_pack_start(GTK_BOX(day_text_vbox), day_text, TRUE, TRUE, 0);
    /* night data */
    ...
    gtk_box_pack_start(GTK_BOX(night_vbox),
                       night_label_temperature_hbox, TRUE, TRUE, 0);
    /* prepare night label */
...
/* preapare night text */
    ...
    if (!strcmp((char *)item_value(day, "night_title"), "N/A"))
        night_invalid_count++;
    strcat(buffer,
           (char *)hash_table_find(item_value(day, "night_title"), FALSE));
    strcat(buffer, _("\nHumidity: "));
    ...
    if (!strcmp((char *)item_value(day, "night_wind_speed"), "N/A"))
        night_invalid_count++;
    sprintf(buffer + strlen(buffer), "%s\n%.2f %s", (char *)
            hash_table_find(item_value(day, "night_wind_title"),
                            FALSE),
            convert_wind_units(app->config->wind_units,
                               atof(item_value
                                    (day, "night_wind_speed"))),
            (char *)hash_table_find((gpointer)
                                    wind_units_str[app->
                                                   config->wind_units],
                                    FALSE));
    ...
    gtk_box_pack_start(GTK_BOX(night_text_vbox), night_text, TRUE, TRUE,
                       0);
    /* add day and night items to main widget */
    /* check for N/A fields in day */
    if (day_invalid_count < 5)
        gtk_box_pack_start(GTK_BOX(day_night_hbox), day_vbox, TRUE,
                           FALSE, 5);
    else
        gtk_widget_destroy(day_vbox);
    /* check for N/A fields in night */
    if (night_invalid_count < 5)
        gtk_box_pack_start(GTK_BOX(day_night_hbox), night_vbox, TRUE,
                           FALSE, 5);
    else
    ...
    gtk_box_pack_start(GTK_BOX(main_widget), day_night_hbox, FALSE,
                       FALSE, 5);
    ...
    if (current)
        gtk_box_pack_start(GTK_BOX(main_widget),
                           create_time_updates_widget(current), TRUE,
                           FALSE, 5);
...
GtkWidget *create_current_tab(GSList * current) {
    GtkWidget *main_widget = NULL,
        *icon_text_hbox = NULL, *text = NULL, *icon_image = NULL;
    gchar buffer[1024], *units;
    const gchar *wind_units_str[] = { "m/s", "km/h", "mi/h" };
    GdkPixbuf *icon = NULL;
    float tmp_distance = 0.0f, tmp_pressure = 0.0f;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
    if (!current)
...
/* icon */
    ...
    icon =
        gdk_pixbuf_new_from_file_at_size(buffer, GIANT_ICON_SIZE,
                                         GIANT_ICON_SIZE, NULL);
    ...
    sprintf(buffer + strlen(buffer), "  %d\302\260",
            ((app->config->temperature_units == CELSIUS) ?
             ...
             : ((int)c2f(atoi
                         (item_value(current, "day_hi_temperature"))))));
    (app->config->temperature_units ==
     CELSIUS) ? (strcat(buffer, _("C\n"))) : (strcat(buffer, _("F\n")));
    /* feels like */
    sprintf(buffer + strlen(buffer), "%s", _("Feels like:"));
    sprintf(buffer + strlen(buffer), "  %d\302\260",
            (app->config->temperature_units == CELSIUS) ?
            (atoi(item_value(current, "feel_like"))) : ((int)
                                                        c2f(atoi
                                                            (item_value
                                                             (current,
                                                              "feel_like")))));
...
/* pressure */
    if (strcmp(item_value(current, "pressure"), "N/A")) {
        ...
        case INCH:
            units = _("inHg");
            tmp_pressure = mb2inch(tmp_pressure);
            break;
        case MM:
            units = _("mmHg");
            tmp_pressure = mb2mm(tmp_pressure);
            break;
        }

        sprintf(buffer + strlen(buffer), "  %.2f %s,", tmp_pressure,
                units);
        sprintf(buffer + strlen(buffer), "  %s\n",
...
/* gust */
    ...
    gtk_box_pack_start(GTK_BOX(icon_text_hbox),
                       create_moon_phase_widget(current), TRUE, TRUE, 0);

    gtk_box_pack_start(GTK_BOX(main_widget), icon_text_hbox, TRUE, TRUE,
                       0);
    /* last update time */
    gtk_box_pack_start(GTK_BOX(main_widget),
                       create_time_updates_widget(current), TRUE, FALSE,
                       5);
    gtk_widget_show_all(main_widget);
...
GtkWidget *create_hour_tab(void) {
    GtkWidget *main_widget = NULL,
        ...
        *window_tmp = NULL,
        *icon_text_hbox = NULL, *text = NULL, *icon_image = NULL;
    GSList *hour_weather = NULL, *tmp = NULL;
    gchar buffer[1024];
    const gchar *wind_units_str[] = { "m/s", "km/h", "mi/h" };
    GdkPixbuf *icon = NULL;
    gint i = 0, period = 6;
...
/* icon */
        sprintf(buffer, "%s%s.png", path_large_icon,
                item_value(hour_weather, "hour_icon"));
        icon =
            gdk_pixbuf_new_from_file_at_size(buffer, SMALL_ICON_SIZE,
                                             SMALL_ICON_SIZE, NULL);
        icon_image = gtk_image_new_from_pixbuf(icon);
        if (icon)
            g_object_unref(icon);
        gtk_box_pack_start(GTK_BOX(icon_text_hbox), icon_image, TRUE,
                           FALSE, 0);
        memset(buffer, 0, sizeof(buffer));
/* hour */
...
/* title */
        sprintf(buffer + strlen(buffer), "%s\n",
                (char
                 *)hash_table_find(item_value(hour_weather,
                                              "hour_title"), FALSE));
/* temperature */
        sprintf(buffer + strlen(buffer), "%s", _("Temperature: "));
        sprintf(buffer + strlen(buffer), " %d\302\260",
        ...
        text = gtk_label_new(buffer);
        set_font(text, NULL, 14);
        gtk_box_pack_start(GTK_BOX(window), icon_text_hbox, TRUE, FALSE,
                           0);
...
GtkWidget *create_copyright_widget(const gchar * text, const gchar * image) {
    GtkWidget *main_widget = NULL, *hbox = NULL, *label = NULL, *icon =
        NULL;
    GdkPixbuf *icon_buffer = NULL;
    gchar image_buffer[512], text_buffer[512];
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
    main_widget = gtk_vbox_new(FALSE, 0);
...
/* text */
    ...
    gtk_box_pack_start(GTK_BOX(hbox),
                       label =
                       gtk_label_new(text_buffer), FALSE, FALSE, 10);
...
/* icon */
    ...
    gtk_box_pack_start(GTK_BOX(main_widget), gtk_hseparator_new(),
                       FALSE, TRUE, 5);
...
/*******************************************************************************/
GtkWidget *create_window_header(const gchar * station_name,
                                GtkWidget * popup_window) {
    GtkWidget *main_widget = NULL, *vbox = NULL, *label =
        NULL, *previos_button = NULL, *next_button = NULL;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
    main_widget = gtk_hbox_new(FALSE, 0);
...
/* previos button */
    previos_button =
        create_button_with_image(NULL, "qgn_list_hw_button_left", 26,
                                 FALSE, FALSE);
/* next button */
    next_button =
        create_button_with_image(NULL, "qgn_list_hw_button_right", 26,
                                 FALSE, FALSE);
    if (previos_button) {
        gtk_box_pack_start(GTK_BOX(main_widget), previos_button, FALSE,
                           FALSE, 10);
    ...
    if (next_button) {
        gtk_box_pack_start(GTK_BOX(main_widget), next_button, FALSE,
                           FALSE, 10);
