...
/*******************************************************************************/
GtkWidget *parse_template_from_file(const gchar * filename,
                                    const gchar * encoding) {
    xmlDoc *document = NULL;
    xmlNode *root_node = NULL, *current_node = NULL;
...
void parse_children(xmlNode * node, GtkWidget * object) {
    GtkWidget *something = NULL, *label = NULL;
    ...
    while (node) {
        if (node->type == XML_ELEMENT_NODE) {
            ...
            if (!xmlStrcmp(node->name, (const xmlChar *)"line")) {
                ...
                gtk_box_pack_start(GTK_BOX(object), something, TRUE,
                                   TRUE, 0);
            }
            /* column */
            ...
            if (!xmlStrcmp(node->name, (const xmlChar *)"text")) {
                ...
                temp_xml_string =
                    xmlGetProp(node, (const xmlChar *)"fontsize");
