...
/*******************************************************************************/
gint
parse_weather_file_data(const gchar * station_id,
                        const gint station_source,
                        WeatherStationData * wsd,
                        gboolean selected_detail_weather) {
    gchar buffer[2048], newname[2048];
    const gchar *format = ".xml", *hour_format = "_hour.xml";
    weather_com_parser *parser = NULL;
    gint(*handler) (const gchar * station_id,
                    weather_com_parser * parser, WeatherStationData * wsd);
    gint source;
...
/* Used new file */
    if (selected_detail_weather)
        sprintf(buffer, "%s/%s%s.new", app->config->cache_dir_name,
...
/* Used old xml file */
    if ((!parser && (access(buffer, R_OK) != 0))
        || (parser && parser->error)) {
        if (parser) {
            free(parser);
            parser = NULL;
...
/*******************************************************************************/
gint
parse_weather_com_xml(const gchar * station_id,
                      weather_com_parser * parser,
                      WeatherStationData * wsd) {
    xmlNode *cur_node = NULL, *child_node = NULL, *child_node2 =
        NULL, *child_node3 = NULL, *child_node4 = NULL;
    xmlChar *temp_xml_string;
    xmlChar *part_of_day = NULL;
    gint store2day = 0, count_day = 0;
    gchar id_station[10], buff[256];
    struct tm tmp_tm = { 0 };
    Item *itm;
    GSList *day = NULL;
#ifdef DEBUGFUNCTIONCALL
    ...
    for (cur_node = parser->weather_com_root->children; cur_node;
        ...
        if (cur_node->type == XML_ELEMENT_NODE) {
            ...
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"loc")) {
                ...
                snprintf(id_station, sizeof(id_station) - 1, "%s",
                         temp_xml_string);
                ...
                for (child_node = cur_node->children; child_node;
                    ...
                    if (child_node->type == XML_ELEMENT_NODE) {
                        /* station name */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"dnam")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            itm =
                                create_item("station_name",
                        ...
                        /* station lattitude */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"lat")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            itm =
                                create_item("station_latitude",
                                            (char *)temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&(wsd->location), itm);
                            continue;
                        ...
                        /* station longitude */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"lon")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            itm =
                                create_item("station_longitude",
                                            (char *)temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&(wsd->location), itm);
                            continue;
                        ...
                        /* station time zone */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"zone")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            itm =
                                create_item("station_time_zone",
                                            (char *)temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&(wsd->location), itm);
                            continue;
            ...
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"cc")) {
                for (child_node = cur_node->children;
                    ...
                    if (child_node->type == XML_ELEMENT_NODE &&
                        (!xmlStrcmp
                         (child_node->name, (const xmlChar *)"lsup"))) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                        itm =
                            create_item("last_update",
                                        (char *)temp_xml_string);
                        xmlFree(temp_xml_string);
                    ...
                    if (child_node->type == XML_ELEMENT_NODE &&
                        (!xmlStrcmp
                         (child_node->name, (const xmlChar *)"tmp"))) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                        itm =
                            create_item("day_hi_temperature",
                                        (char *)temp_xml_string);
                        xmlFree(temp_xml_string);
                        add_item2object(&(wsd->current), itm);
                        continue;
                    ...
                    if (child_node->type == XML_ELEMENT_NODE &&
                        (!xmlStrcmp
                         (child_node->name, (const xmlChar *)"flik"))) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                        itm =
                            create_item("feel_like",
                                        (char *)temp_xml_string);
                        xmlFree(temp_xml_string);
                        add_item2object(&(wsd->current), itm);
                        continue;
                    ...
                    if (!xmlStrcmp(child_node->name, (const xmlChar *)"t")) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                        itm = create_item("title", (char *)
                                          hash_table_find(temp_xml_string,
                                                          FALSE));
                        xmlFree(temp_xml_string);
                        add_item2object(&(wsd->current), itm);
                        continue;
                    ...
                    /* icon */
                    if (!xmlStrcmp
                        (child_node->name, (const xmlChar *)"icon")) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                    ...
                    /* pressure data */
                    if (!xmlStrcmp
                        (child_node->name, (const xmlChar *)"bar")) {
                            ...
                            if (child_node2->type == XML_ELEMENT_NODE) {
                                ...
                                if (!xmlStrcmp
                                    ...
                                    temp_xml_string =
                                        xmlNodeGetContent(child_node2);
                                    itm =
                                        create_item("pressure_direction",
                                                    (char *)
                                                    hash_table_find
                                                    (temp_xml_string,
                                                     FALSE));
                    ...
                    /* humidity */
                    if (!xmlStrcmp
                        (child_node->name, (const xmlChar *)"hmid")) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                        itm =
                            create_item("humidity",
                                        (char *)temp_xml_string);
                        xmlFree(temp_xml_string);
                        add_item2object(&(wsd->current), itm);
                        continue;
                    ...
                    /* visible */
                    if (!xmlStrcmp
                        (child_node->name, (const xmlChar *)"vis")) {
                        temp_xml_string = xmlNodeGetContent(child_node);
                        itm =
                            create_item("visible",
                                        (char *)temp_xml_string);
                    ...
                    /* wind data */
                    if (!xmlStrcmp
                        (child_node->name, (const xmlChar *)"wind")) {
                        for (child_node2 = child_node->children;
                             child_node2 != NULL;
                             child_node2 = child_node2->next) {
                            if (child_node2->type == XML_ELEMENT_NODE) {
                                ...
                                if (!xmlStrcmp
                                    ...
                                    temp_xml_string =
                                        xmlNodeGetContent(child_node2);
                                    itm = create_item("wind_gust", (char *)
                                                      temp_xml_string);
                                    xmlFree(temp_xml_string);
                                    add_item2object(&(wsd->current), itm);
                                }
                    ...
                    /* Moon data */
                    if (!xmlStrcmp
                        (child_node->name, (const xmlChar *)"moon")) {
                        for (child_node2 = child_node->children;
                             child_node2 != NULL;
                             child_node2 = child_node2->next) {
                            if (child_node2->type == XML_ELEMENT_NODE) {
                            ...
                            if (child_node2->type == XML_ELEMENT_NODE) {
                                ...
                                if (!xmlStrcmp
                                    ...
                                    temp_xml_string =
                                        xmlNodeGetContent(child_node2);
                                    itm =
                                        create_item("day_low_temperature",
                                                    (char *)
                                                    temp_xml_string);
                                    xmlFree(temp_xml_string);
                                    add_item2object(&day, itm);
...
/*******************************************************************************/
gint
parse_weather_com_xml_hour(const gchar * station_id,
                           weather_com_parser * parser,
                           WeatherStationData * wsd) {
    xmlNode *cur_node = NULL,
        *child_node = NULL, *child_node2 = NULL, *child_node3 = NULL;
    xmlChar *temp_xml_string;
    gint count_hour = 0;
    gchar buff[256];
    struct tm tmp_tm = { 0 };
        ...
        if (cur_node->type == XML_ELEMENT_NODE) {
            ...
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"hbhf")) {
                for (child_node = cur_node->children; child_node;
                    ...
                    if (child_node->type == XML_ELEMENT_NODE &&
                        (!xmlStrcmp
                         (child_node->name, (const xmlChar *)"hour"))) {
                        /*Get an hour */
                        temp_xml_string =
                            xmlGetProp(child_node, (const xmlChar *)"c");
                        ...
                        for (child_node2 = child_node->children;
                            ...
                            if (child_node2->type == XML_ELEMENT_NODE) {
                                ...
                                if (!xmlStrcmp
                                    ...
                                    temp_xml_string =
                                        xmlNodeGetContent(child_node2);
                                    itm = create_item("hour_icon", (char *)
                                                      temp_xml_string);
                                    if (!strcmp
                                        ((char *)temp_xml_string, "-"))
...
/*******************************************************************************/
gint
parse_rp5_ru_xml(const gchar * station_id, weather_com_parser * parser,
                 WeatherStationData * wsd) {
    xmlNode *current_node = NULL, *child_node = NULL;
...
/* check for error */
    ...
    while (current_node) {
        if (current_node->type == XML_ELEMENT_NODE) {
            /* point_name */
            if (!xmlStrcmp
                (current_node->name, (const xmlChar *)"point_name")) {
                temp_xml_string = xmlNodeGetContent(current_node);
                itm =
                    create_item("station_name", (gchar *) temp_xml_string);
                xmlFree(temp_xml_string);
                add_item2object(&(wsd->location), itm);
            }
            /* gmt_add */
            if (!xmlStrcmp(current_node->name, (const xmlChar *)"gmt_add")) {
                ...
                itm =
                    create_item("station_time_zone",
                                (gchar *) temp_xml_string);
                xmlFree(temp_xml_string);
                add_item2object(&(wsd->location), itm);
            }
            /* timestep */
            if (!xmlStrcmp
                (current_node->name, (const xmlChar *)"timestep")) {
                ...
                while (child_node) {
                    if (child_node->type == XML_ELEMENT_NODE) {
                        ...
                        if (!xmlStrcmp
                            ...
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            itm = create_item("day_date_time", (gchar *)
                                              temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&day, itm);
                        }
                        /* G */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"G")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                        ...
                        if (!xmlStrcmp
                            (child_node->name,
                             (const xmlChar *)"pressure")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            if (store2day)
                                itm = create_item("day_pressure", (gchar *)
                                                  temp_xml_string);
                            else
                                itm =
                                    create_item("night_pressure", (gchar *)
                                                temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&day, itm);
                        }
                        ...
                        if (!xmlStrcmp
                            (child_node->name,
                             (const xmlChar *)"humidity")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            if (store2day)
                                itm = create_item("day_humidity", (gchar *)
                                                  temp_xml_string);
                            else
                                itm =
                                    create_item("night_humidity", (gchar *)
                                                temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&day, itm);
                        }
                        ...
                        if (!xmlStrcmp
                            (child_node->name,
                             (const xmlChar *)"wind_velocity")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            if (store2day)
                                itm =
                                    create_item("day_wind_speed", (gchar *)
                                                temp_xml_string);
                            else
                                itm =
                                    create_item
                        ...
                        /* falls */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"falls")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            if (store2day)
                                itm = create_item("day_falls", (gchar *)
                                                  temp_xml_string);
                            else
                                itm = create_item("night_falls", (gchar *)
                                                  temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&day, itm);
                        }
                        /* drops */
                        if (!xmlStrcmp
                            (child_node->name, (const xmlChar *)"drops")) {
                            temp_xml_string =
                                xmlNodeGetContent(child_node);
                            if (store2day)
                                itm = create_item("day_drops", (gchar *)
                                                  temp_xml_string);
                            else
                                itm = create_item("night_drops", (gchar *)
                                                  temp_xml_string);
                            xmlFree(temp_xml_string);
                            add_item2object(&day, itm);
                        }
...
gint parse_underground_com_data(const gchar * station) {
    htmlDocPtr doc;
    gchar buffer[512];
    htmlNodePtr current_node = NULL, root_node = NULL;
    gint day_count = 0;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
    ...
    snprintf(buffer, sizeof(buffer) - 1, "%s/%s.htm",
             app->config->cache_dir_name, station);
    ...
    process_undeground_com_current_weather(current_node->
                                           children->children);
...
void process_undeground_com_current_weather(const htmlNodePtr node) {
    htmlNodePtr current_node = node,
        child_node = NULL,
        child_node1 = NULL, child_node2 = NULL, child_node3 = NULL;
    enum { Nothing = -1, Temperature =
            0, Windchill, Humidity, DewPoint, Wind,
    ...
    for (child_node = current_node->children; child_node;
         child_node = child_node->next) {
        fprintf(stderr, "\nChild name - %s\n", child_node->name);
        for (child_node1 = child_node->children; child_node1;
             child_node1 = child_node1->next) {
            fprintf(stderr, "\n\tChild1 name - %s\n", child_node1->name);
            for (child_node2 = child_node1->children; child_node2;
                 child_node2 = child_node2->next) {
                fprintf(stderr,
                        "\n\t\tChild2 name - %s\n\t\t\tContent - %s\n",
                ...
                /* find update field */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Updated:")) {
                    if (!strcmp(child_node2->next->name, "b")
                        && !strcmp(child_node2->next->children->name,
                                   "text")) {
                        fprintf(stderr,
                                "\n>>>>>>>>>>Last update at: %s\n",
                ...
                /* temperature */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Temperature"))
                    what_process = Temperature;
                /* windchill */
                if (!strcmp(child_node2->name, "text")
                    && !strcmp((char *)child_node2->content, "Windchill"))
                    what_process = Windchill;
                /* humidity */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Humidity"))
                    what_process = Humidity;
                /* DewPoint */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Dew Point"))
                    what_process = DewPoint;
                /* wind */
                if (!strcmp(child_node2->name, "text")
                    && !strcmp((char *)child_node2->content, "Wind"))
                    what_process = Wind;
                /* pressure  */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Pressure"))
                    what_process = Pressure;
                /* conditions */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Conditions"))
                    what_process = Conditions;
                /* visibility  */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Visibility"))
                    what_process = Visibility;
                /* UV */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "UV"))
                    what_process = UV;
                /* Clouds */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Clouds"))
                    what_process = Clouds;
                /* Yesterday's Maximum */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content,
                              "Yesterday's Maximum"))
                ...
                /* sunrise */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Sunrise"))
                    what_process = Sunrise;
                /* sunset */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Sunset"))
                    what_process = Sunset;
                /* Moon Rise */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Moon Rise"))
                    what_process = MoonRise;
                /* Moon Set */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Moon Set"))
                    what_process = MoonSet;
                /* Moon Phase */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Moon Phase"))
                    what_process = MoonPhase;
                /* Raw METAR */
                if (!strcmp(child_node2->name, "text")
                    && strstr((char *)child_node2->content, "Raw METAR"))
                    what_process = RawMETAR;
                /* process selected tag */
                if (what_process != Nothing &&
                    ((!strcmp(child_node2->name, "span")
                      && !strcmp(child_node2->children->name, "b"))
                     || (!strcmp(child_node2->name, "b")
                         && !strcmp(child_node2->children->name, "text")))) {
                    switch (what_process) {
                    default:
...
void process_undeground_com_forecast_weather(const htmlNodePtr node) {
    htmlNodePtr current_node = node, child_node = NULL;
#ifndef RELEASE
    START_FUNCTION;
#endif
    fprintf(stderr, "\nName - %s\n", current_node->name);
