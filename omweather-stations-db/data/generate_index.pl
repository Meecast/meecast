...
foreach my $country (sort(@$countries)){
	...
	my ($region, $weather_com_code, $rp5_ru_code, $location, $lat, $lon) = (undef);
	...
	while(my $line = <FH>){
		...
		if($line =~ /(.+)=(.+)?=(.+)?=(.+)?=(.+)?/){
		    ($location, $weather_com_code, $rp5_ru_code, $lat, $lon) = ($1, $2, $3, $4, $5);
		    die "Can't get current position in $locationsfile: $!\n" if $location_start == -1;
		    if ($lat < $minlat) {$minlat = $lat;}
		    if ($lat > $maxlat) {$maxlat = $lat;}
		    if ($lon < $minlon) {$minlon = $lon;}
		    if ($lon > $maxlon) {$maxlon = $lon;}
		    print LOCATIONS "$location;$weather_com_code;$rp5_ru_code;$lat;$lon;\n";
		}
	    }
	}
	close FH;
