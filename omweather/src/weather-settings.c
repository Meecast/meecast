...
/*******************************************************************************/
void
add_station_to_user_list(gchar * weather_station_name,
...
void changed_country_handler(GtkWidget * widget, gpointer user_data) {
    ...
    GtkWidget *config = NULL, *countries = NULL, *states = NULL;
    GtkTreeModel *model;
    GtkTreeIter iter;
    gchar *country_name = NULL;
    long regions_start = -1, regions_end = -1, regions_number = 0;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
    if (!(user_data))
    ...
    if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(countries), &iter)) {
        ...
        gtk_tree_model_get(model, &iter, 0, &country_name,
                           1, &regions_start, 2, &regions_end, -1);
        if (app->regions_list)
            gtk_list_store_clear(app->regions_list);
        app->regions_list
            =
...
void changed_state_handler(GtkWidget * widget, gpointer user_data) {
    ...
    GtkWidget *config = GTK_WIDGET(user_data),
        *states = NULL, *stations = NULL;
    GtkTreeModel *model = NULL;
    GtkTreeIter iter;
    gchar *state_name = NULL;
    long stations_start = -1, stations_end = -1;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
#endif
    list =
...
/* clear locations list */
    ...
    if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(states), &iter)) {
        ...
        gtk_tree_model_get(model, &iter, 0, &state_name,
                           1, &stations_start, 2, &stations_end, -1);
        /* clear locations list */
        if (app->stations_list)
            gtk_list_store_clear(app->stations_list);
...
void delete_station_handler(GtkButton * button, gpointer user_data) {
    GtkWidget *dialog = NULL,
        *config = GTK_WIDGET(user_data), *rename_entry = NULL;
    GtkTreeView *station_list_view = NULL;
    GtkTreeIter iter;
    gchar *station_selected = NULL, *station_name = NULL, *station_code =
        NULL;
    GtkTreeModel *model;
...
/*******************************************************************************/
void
station_list_view_select_handler(GtkTreeView * tree_view,
                                 gpointer user_data) {
    GtkTreeIter iter;
    gchar *station_selected = NULL, *station_name = NULL, *station_code =
        NULL;
...
/*******************************************************************************/
void
weather_window_settings(GtkWidget * widget, GdkEvent * event,
                        gpointer user_data) {
    gint day_number = (gint) user_data; /* last looking day on detail window */

    GtkWidget *window_config = NULL, *notebook = NULL, *vbox = NULL,
        ...
        *visual_tab = NULL,
        *display_tab = NULL,
        *units_tab = NULL, *update_tab = NULL, *alerts_tab = NULL;
#ifndef RELEASE
    char tmp_buff[1024];
#endif
#ifdef DEBUGFUNCTIONCALL
...
/* Pack buttons to the buttons box */
    ...
    gtk_box_pack_start(GTK_BOX(buttons_box), apply_button, FALSE, FALSE,
                       0);
...
/*******************************************************************************/
void
apply_button_handler(GtkWidget * button, GdkEventButton * event,
                     gpointer user_data) {
    struct lists_struct *list = NULL;
    GtkTreeModel *model;
    GtkTreeIter iter;
    ...
    GtkWidget *config_window = GTK_WIDGET(user_data),
        ...
        *validtime = NULL,
        *update_time = NULL,
        *weather_source = NULL, *download_after_connection = NULL,
#if defined(OS2008) || defined(DEBUGTEMP)
        *use_sensor = NULL, *display_at = NULL, *sensor_update_time = NULL,
#endif
        *temperature = NULL,
        *pressure_mb = NULL,
        *pressure_inch = NULL,
        ...
        *countries = NULL,
        *states = NULL,
        *theme_override = NULL, *selected_icon_set = NULL, *stations =
        NULL;
...
/* Weather data source */
    ...
    list = (struct lists_struct *)
        g_object_get_data(G_OBJECT(config_window), "list");
    if (weather_source && list) {
        ...
        active_index =
            gtk_combo_box_get_active(GTK_COMBO_BOX(weather_source));
    ...
    if (!strcmp((char *)pressed_button, "add_name")) {
        ...
        if (!lookup_and_select_station
            ...
            hildon_banner_show_information(app->main_window,
                                           NULL,
                                           _
                                           ("No one station found! May be you don't have installed omweather-*-station-db packet"));
            gtk_entry_set_text((GtkEntry *) station_name_entry, "");
        }
    } else {
        if (!strcmp((char *)pressed_button, "add_code")) {
            station_code_entry =
                lookup_widget(config, "station_code_entry");
            station_code_invalid =
                check_station_code(app->config->weather_source,
                                   gtk_entry_get_text((GtkEntry *)
                                                      station_code_entry));
            if (!station_code_invalid) {
                add_station_to_user_list(g_strdup
                                         (gtk_entry_get_text((GtkEntry *)
                                                             station_code_entry)),
                ...
                g_object_set_data(G_OBJECT(config),
                                  "need_update_weather", (gpointer) 1);
            } else {
                /* Need Error window */
            }
        } else {
...
GtkWidget *create_locations_tab(GtkWidget * window) {
    ...
    GtkWidget *countries = NULL, *states = NULL, *stations = NULL,
#ifdef OS2008
        *chk_gps = NULL,
#endif
        *left_table = NULL,
        ...
        *add_station_button2 = NULL, *apply_button = NULL, *rename_entry =
        NULL;
    gint i = 0;
#ifdef DEBUGFUNCTIONCALL
    START_FUNCTION;
...
/* Pack Up, Down and Delete buttons */
    gtk_box_pack_start(GTK_BOX(up_down_delete_buttons_vbox),
                       up_station_button, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(up_down_delete_buttons_vbox),
                       delete_station_button, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(up_down_delete_buttons_vbox),
...
/* GPS */
    ...
    gtk_box_pack_start(GTK_BOX(left_right_hbox), right_table, TRUE,
                       TRUE, 0);
    gtk_table_attach_defaults(GTK_TABLE(right_table),
                              gtk_label_new(_("Add")), 1, 2, 0, 1);
    /* source label */
    gtk_table_attach_defaults(GTK_TABLE(right_table),
    ...
    if (app->countrys_list) {
        gtk_combo_box_set_active(GTK_COMBO_BOX(countries),
                                 get_active_item_index((GtkTreeModel *)
                                                       ...
                                                       TRUE));
        /* fill states list */
        g_idle_add((GSourceFunc) changed_country_process,
                   (gpointer) window);
        /* fill stations list */
